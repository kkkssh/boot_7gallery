...
//		String path ="c:\\upload";
		...
		//파일업로드
		for(MultipartFile f:dto.getPics()) {		//첨부파일이 여러개 일 때 반복
...
//				String prefix = ofilename.substring(0, ofilename.lastIndexOf("."));   //원래의 파일이름
	...
	//////////////3-f. 좋아요 ///////////////////////////////////////////////////////
	@Override
	public Map<String, String> processHeartCount(String data) {		//data는 json문자열
	...
	try {
		...
		HeartResponseDTO responseDTO =
				HeartResponseDTO.builder().alarm(alarm).hearts(cnt).writer(heart.getWriter()).build();
